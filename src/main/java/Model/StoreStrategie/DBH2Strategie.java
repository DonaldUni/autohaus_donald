...
public class DBH2Strategie implements StoreStrategie {
    ...
    public DBH2Strategie() {
        ...
        try {
            ...
            connection = DriverManager.getConnection("jdbc:h2:Datenbank.db");
    ...
    public void createClientList(Connection con){
        try (PreparedStatement pstmt = con.prepareStatement("CREATE TABLE IF NO EXISTS ClientList "
                +"(id INTEGER, firstName TEXT, lastName TEXT, address TEXT);")){

            pstmt.executeUpdate();
            System.out.println("ClientList successfull created");
